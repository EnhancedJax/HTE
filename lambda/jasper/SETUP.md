...
- 我們**部署到 AWS** 的只有：**你的 Lambda 函數**（Jasper 的程式）。Lambda 透過網路呼叫 Pinecone API，把向量寫進已選在 AWS region 的 Pinecone Index。
...
| **你的 Lambda（Jasper）** | 你部署到自己的 AWS 帳號 | AWS Console → Lambda → 上傳程式、設環境變數 |
...
**只需要上傳「Jasper 這支 Lambda」的程式碼**，也就是專案裡的 `lambda/jasper/` 這個資料夾的內容。**不用**上傳整個專案（例如 platform、python 等），產品其他部分之後再做即可。

上傳的 zip 裡只要包含這三樣（都是 `lambda/jasper/` 底下的）：
- `dist/`（編譯後的 JS）
- `node_modules/`（只有執行時需要的套件）
- `package.json`

#### 方法一：一鍵打包（建議，macOS / Linux）

在終端機執行（先進入 **`lambda/jasper`** 目錄；若從未安裝過依賴，先執行一次 `npm install`）：

```bash
cd lambda/jasper
npm install          # 第一次或之後有改 package.json 時執行即可
npm run package:lambda
```

會在同目錄下產生 **`jasper-lambda.zip`**。到 AWS Lambda 主控台 → 你的函數 → **程式碼** → **上傳自** → 「.zip 檔案」→ 選這個 `jasper-lambda.zip` 即可。

#### 方法二：手動打包

1. 進入 Jasper 目錄並安裝依賴、編譯、精簡依賴：
   ```bash
   cd lambda/jasper
   npm install
   npm run build
   ...
   ```
2. 把 **`dist/`、`node_modules/`、`package.json`** 三個打進一個 zip（不要包一層 `lambda/jasper` 資料夾；zip 解開後就要看到 `dist`、`node_modules`、`package.json`）。
   - macOS / Linux 範例：`zip -r jasper-lambda.zip dist node_modules package.json`
3. 在 Lambda 主控台：
   - **程式碼** → **上傳自** → 「.zip 檔案」→ 選這個 zip。
